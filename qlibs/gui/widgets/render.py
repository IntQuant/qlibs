...
class DefaultRenderer:
    ...
    def render_node(self, node):
        self.drawer.add_rectangle(*node.position, *node.size, color=(1, 1, 1, 0.2))
        if node.type == "button":
            pos = MVec(node.position + node.size // 2)
            pos.x -= self.font_render.calc_size(node.name) // 2
            pos.y -= self.font_render.calc_height(node.name) // 2
            self.queue_text(node.name, *pos)
