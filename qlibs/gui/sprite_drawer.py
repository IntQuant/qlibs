...
class SpriteDrawer:
    ...
    def add_sprite_rect(self, id_, x, y, w, h):
        data = [
            #First triangle
            x,   y,   0, 0, id_,
            x+w, y+h, 1, 1, id_,
    ...
    def add_sprite_centered(self, id_, x, y, w, h):
        self.add_sprite_rect(id_, x - w//2, y - h//2, w, h)

    def add_sprite_rotated(self, id_, x, y, w, h, r):
        rot = math.atan2(w, h) + r + math.pi
        d = math.sqrt(w*w + h*h) / 2

        sin = math.sin #TODO: this is quite slow, lower precision might suffice
        cos = math.cos
    ...
    def render(self, mvp=Matrix4(IDENTITY), reset=True):
        ...
        vao = self.ctx.simple_vertex_array(self.program, buffer, "pos", "tpos")
        self.texture.use()
