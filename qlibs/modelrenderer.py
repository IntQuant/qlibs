...
class RenderableModel():
    ...
    def prepare(self):
        ...
        for mat, data in self.model.iter_materials_textured(*modelloader.FORMAT_TEXTURES):
            mat = self.model.materials[mat.mat_name]
            mat.process()
            vbo = self.ctx.buffer(data.tobytes())
            vao = self.ctx.simple_vertex_array(TEXTURE_RENDER_PROGRAM, vbo, 'in_vert', "normal", "uv")
